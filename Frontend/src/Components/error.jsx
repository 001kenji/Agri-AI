        ...
        return (
          <div className=" flex flex-col md:flex-row bg-slate-100 min-h-screen w-full h-full justify-center align-middle">
              <div className=" md:w-1/2 flex py-3 flex-col bg-transparent justify-center align-middle h-fit min-h-[250px]  gap-4 content-center">
                ...
                <h1 id="BigProppin" className=" text-left w-fit text-xl sm:text-3xl px-1 flex flex-row font-semibold mx-auto text-slate-900  h-fit gap-2">
                  <p className=" text-indigo-600 mx-0 p-0 font-semibold">Oops</p> Something went wrong.</h1>
                  
                  <button onClick={() => window.history.back()} to="/home" className="tooltip rounded-sm tooltip-left md:tooltip-top py-2 px-4 text-lg my-auto font-[PoppinsN] text-slate-950 border-[2px] w-fit mx-auto hover:text-slate-50 dark:hover:text-slate-50 cursor-pointer z-30 bg-transparent bg-opacity-35 hover:bg-sky-700 transition-all duration-500 hover:bg-opacity-40 dark:text-slate-300  border-sky-600">Go Back</button>
              </div>
