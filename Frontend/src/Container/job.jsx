...
const Jobs =({logout,isAuthenticated,UploadFile}) => {
    ...
    const requestWsStream = (msg = null,body = null) => {    
        ...
        WsDataStream.current.onmessage = function (e) {
          var data = JSON.parse(e.data)
            ...
            if(data.type == 'PostJob'){
                ...
                SetChatLog(data.message[0].ChatLog)
                dispatch({
                    type : ChatLogReducer,
                    payload : data.message[0].ChatLog != 'null' && data.message[0].ChatLog != null ? data.message[0].ChatLog : []
                }
                )
            }else if(data.type == 'MyjobLogChats'){
                    if(data.message[0] == 'true'){
                        SetIsStartChat(false)
                        SetRoomName(data.message[1][0].group_name)
                        SetSenderIdVal([data.message[1][0].SenderId,data.message[1][0].RecieverId])
                    }else {
                        SetIsStartChat(true)
                        var custom = `${UserId}${data.message[1][0].id}`
                        var emailval = data.message[1][0].email
                        var customdiv = {
                             [emailval] : {
                                'name' : data.message[1][0].name,
                                'about' : data.message[1][0].about,
                                'id' : data.message[1][0].id,
                                'ProfilePic' : `${import.meta.env.VITE_APP_API_URL}/media/${data.message[1][0].ProfilePic}`
                            },
                            [Email] : {
            ...
            }else if(data.type == 'StartNewChat'){
                if(data.message == 'True'){
                    SetIsStartChat(false)
                    CallCreateChatRoom(HeaderLogValChats.group_name,UserId,HeaderLogValChats.id)
                }
        ...
        if(WsDataStream.current.readyState === WsDataStream.current.OPEN){            
            ...
            }else if(msg == 'MyjobLogChats'){
                WsDataStream.current.send(
                    JSON.stringify({
                        'message' : 'MyjobLogChats',
                        'SenderId' : User != null ? User.id : 'null',
                        'RecieverId' : body
                    })
                    )
            }else if (msg == 'StartNewChat'){
                WsDataStream.current.send(
                    JSON.stringify({
                        'message' : 'StartNewChat',
                        'RecieverId' : HeaderLogValChats.id,
                        'SenderId' : User != null ? User.id : 'null',
                        'group_name' : HeaderLogValChats.group_name,
                        'Details' : HeaderLogValChats.Details,
                        'email' : Email
                    })
                )
    ...
    const ChatLogMapper = ChatLog.map((items,i) => {
        ...
        return (
            <div  key={i} className={`dropdown ${DisplayMore ? ' z-20' : ' z-40'}  bg-transparent relative flex px-2 min-w-fit max-w-[280px] sm:max-w-fit  bg-opacity-90 w-fit my-1 mx-2 ${items.email == Email ? 'chat chat-end  sm:dropdown-top float-right ml-auto' : 'chat chat-start sm:dropdown-top  mr-auto '}  flex-col gap-1`}>
                <span className={` ${items.email == Email ? 'text-sky-700 dark:text-slate-100': ' text-red-700 dark:text-yellow-500'} chat-header  font-semibold`}>{items.name}</span>
                <button tabIndex={i} className= {` overflow-hidden absolute  z-[100%] ${items.email == Email ?  'right-full' : ' left-full'}  top-8 self-center items-center p-2 text-sm font-medium text-center text-gray-900 bg-slate-200 rounded-lg hover:bg-gray-100 focus:ring-4 focus:outline-none dark:text-white focus:ring-gray-50 dark:bg-gray-900 dark:hover:bg-gray-800 dark:focus:ring-gray-600 `} type="button">
                <IoMdMore  title="more" className={` text-base  cursor-pointer my-auto  sm:text-xl`} />

                </button>
              
                <div className={`chat-bubble z-40 min-w-fit  shadow-md w-[200px] ${items.email == Email ? ' flex-col shadow-red-500  dark:shadow-pink-500  ' : ' bg-slate-900 dark:text-slate-900 dark:bg-slate-800 shadow-purple-500 dark:shadow-green-500 text-slate-100 flex-col'} gap-2 w-full flex `}>
                    <div className= {` border-[1px] rounded-sm p-2 border-slate-600 ${items.ReplyChat != 'null'&& items.ReplyChatName !='null' && items.ReplyChat && items.Status != 'deleted' ? ' flex' : 'hidden'}  flex flex-col max-w-[200px] sm:max-w-[300px]  w-fit justify-start gap-1 my-3 `}   >
                        <span className={` ${items.email == Email ? 'text-sky-600 dark:text-slate-100': ' text-red-700 dark:text-yellow-500'} chat-header  font-semibold flex flex-row gap-3`}>{items.ReplyChatName} : <p className=" italic text-orange-500 text-[x-small] md:text-sm">Reply</p></span>
                        <textarea className={` border-none bg-transparent ${items.email == Email ? ' mr-auto' : ' ml-auto'} w-full text-left  break-words text-slate-100   h-fit max-h-[50px] min-h-[30px] sm:max-h-[100px] min-w-full  rounded-sm font-mono text-sm p-2 resize-y cursor-text`} disabled value={items.ReplyChat} ></textarea>
                    </div>
                    ...
                    <audio  src={`${import.meta.env.VITE_WS_API}/media/${items.src}`}  controls title="Uploaded Audio"  className={`cursor-pointer ${items.email == Email ? ' mr-2' : ' ml-2'} px-2  ${items.upload == 'audio' && items.src != 'null' ? 'flex' : ' hidden'} border-slate-800 min-h-[40px] min-w-[200px]  w-[90%] sm:min-w-[300px]  h-fit`} ></audio>
                    <img onClick={() => OpenImage(items.src)} src={`${import.meta.env.VITE_WS_API}/media/${items.src}`}  title="Uploaded image"   className={`cursor-pointer ${items.email == Email ? ' mr-3' : ' ml-3'}  ${items.upload == 'image' && items.src != 'null' ? ' flex' : 'hidden'}  rounded-sm  min-w-fit border-none my-auto w-fit max-w-[200px] h-fit max-h-[100px] lg:max-h-[150px] `}  alt="" />
                    
                    <blockquote className={` max-w-[200px]  sm:max-w-[300px]  break-words ${items.email != Email && (items.Status != 'deleted' || !userDel) &&items.action != 'deleted from all'  ? ' dark:text-amber-500 text-sky-600' :items.email == Email && (items.Status != 'deleted' || !userDel) &&items.action != 'deleted from all' ? ' text-yellow-400 dark:text-purple-500 ' : (userDel && items.action == 'deleted from me') || items.action == 'deleted from all'   ? 'text-pink-500' : '' } font-mono text-sm `} > 
                    ...
                    <div className={` ${items.email == Email ? ' flex-row' : ' flex-row-reverse'} gap-2 sm:gap-3 md:gap-4 w-full flex `}>
                        <small className={`  italic font-semibold font-mono w-full ${items.email == Email ? ' text-left dark:text-cyan-500 pr-3':' dark:text-orange-500 text-right text-green-500'}  `}>{items.time}</small>
                        { items.action != 'deleted from all' && !userDel || items.Status == 'send' ? 
                        <FaCheckDouble className={` ${items.email == Email ? 'text-lime-500 dark:text-green-500' : ' text-red-600 '}  text-sm`}/> :
                ...
                <div tabIndex={i}  className= {` ${i == 0 ? ' translate-y-40 ' : 'translate-y-20'} z-50  font-semibold dropdown-content  bg-white divide-y divide-gray-100 rounded-lg shadow w-40 dark:bg-gray-700 dark:divide-gray-600 `}>
                    <ul className=" text-sm text-gray-700 dark:text-gray-200" aria-labelledby="dropdownMenuIconButton">
                        ...
                        <li>
                            <span data-tip="Copied" onClick={() => CopyChat(items.text,i)} className= {` w-full ${tooltipVal == i ? 'tooltip tooltip-open' : ''} text-left  ${items.email == Email ?'tooltip-left' : 'tooltip-right'}  block cursor-pointer px-4 py-2 hover:bg-gray-300 dark:hover:bg-gray-600 dark:hover:text-white `}>Copy</span>
                        </li>
                        ...
                        <li className={` z-50 ${items.email == Email && items.action != 'deleted from me' && items.action != 'deleted from all' ? ' w-full flex' : ' hidden'} `} >
    ...
    const SendChat =(action='null',textDel= 'null',Deltime = 'null',Delemail = 'null',Delname = 'null') => {
        ...
        if(ChatMessageRef.current != null && Generaldata.chatMessage != '' || action != 'null'){
    ...
    const ChatMapper = ChatList.map((items,i) => {
        ...
        return (
            <div onClick={() => CallCreateChatRoom(items.group_name,items.SenderId,items.RecieverId)} className=" px-3 py-1 hover:shadow-md dark:hover:shadow-slate-500 transition-all duration-300   hover:shadow-slate-800 cursor-pointer hover:bg-slate-200 dark:hover:bg-slate-900 flex flex-row justify-start gap-2" key={i}>
                ...
                <div className=" text-sm sm:text-base flex flex-col gap-2">
                    <span className=" font-semibold" id="PoppinN">{name}</span>
                    <span>{about}</span>
                </div>
            </div>
        )
        ...
        <div className={` w-full md:h-screen overflow-x-hidden  h-full ${Theme}`}>
            ...
            <div className=" overflow-visible w-full h-full  min-h-full">
                ...
                <div className=" h-fit  flex flex-col bg-slate-100 dark:bg-slate-800 font-semibold dark:text-slate-100  gap-2 w-full ">
                        ...
                        <div className= {`shadow-lg shadow-orange-600  dark:bg-slate-800 bg-slate-100 ring-1 ring-sky-600 dark:ring-orange-500 fixed  z-50 mt-20 ${ShowChatDisp ? 'flex flex-col' : 'hidden'} left-1 sm:left-4 `}>
                            <IoMdClose  onClick={() => SetShowChatDisp(false)}  className= {` ml-auto mr-3 hover:bg-red-700 hover:text-slate-800 transition-all duration-500 mt-2 rounded-sm swap-off  fill-current text-red-700 font-semibold cursor-pointer text-2xl `} />

                            <div className={`flex flex-col gap-2 py-2 max-h-[250px] overflow-auto   rounded-sm  w-fit mx-auto mr-2 min-w-[200px] max-w-[280px] h-fit   `}>
                                {ChatMapper}
                            </div>

                            
                        </div>
                        <div className={` mb-8 ${ShowChatComponent == true ? 'flex' : ' hidden'} fixed  mb-3 z-50 dark:bg-slate-800 top-[10rem] bg-slate-100 min-h-[400px] min-w-[250px] max-w-[400px] sm:max-w-[500px] flex `}>
                                <section className={` overflow-x-hidden  mt-5 w-[100%]  rounded-sm border-green-600 shadow-md shadow-green-600 border-[1px] flex flex-col justify-between   m-auto md:mx-auto bg-transparent dark:text-slate-100  h-[90%]`}>
                                    {/* top sticky header */}
                                    <div className=" bg-slate-100 dark:bg-slate-800 top-0 m-2 sticky z-40 scroll-mt-4  min-h-[60px] flex flex-row  max-h-[70px]">
                                        <div className={` ${!DisplayMore ? 'z-30' : ' z-20' } flex flex-row justify-around  h-full gap-0 w-[90%] mr-auto px-1`} >
                                            <img onClick={() => {OpenImagePrivate(HeaderLogValChats.ProfilePic)}} src={`${HeaderLogValChats.ProfilePic}`}  className={`flex cursor-pointer rounded-full mr-auto  my-auto w-16   h-16 `}  alt="" />
                                            <div className=" px-1 mr-auto flex flex-col h-full justify-center gap-1 w-[90%]">
                                                <div className={` flex flex-row gap-2 align-middle w-fit`}>
                                                    <input id=""  className=" flex h-fit my-auto text-sm outline-none bg-transparent text-left w-full max-w-full  border-none text-ellipsis" readOnly type="text" value={`Name:  ${HeaderLogValChats.Name}`} />
                                                </div>
                                                <input id=""  className={` flex h-fit text-sm my-auto outline-none bg-transparent text-left w-full max-w-full  border-none text-ellipsis`} readOnly type="text" value={`About:  ${HeaderLogValChats.about}`} />
                                            </div>                                        
                                        </div>
                                        <div className=" dark:text-slate-100 w-[10%] max-w-[300px] flex align-middle justify-center">
                                                <IoMdMore onClick={() => SetDisplayMore((e) => !e )} title="more" className={` text-2xl ${DisplayMore ? '' : ' z-20'}  cursor-pointer hover:text-3xl hover:md:text-4xl my-auto text-2xl md:text-3xl`} />
                                                <div className={` absolute mr-[90%]   bg-slate-900 bg-opacity-90 min-w-[300px] max-w-[300px] transition-all duration-500 h-fit min-h-[100px]  ${DisplayMore ? ' flex flex-row z-30 opacity-100' : ' z-0 opacity-0 hidden '}`}>
                                                        <div className=" my-2 flex flex-col gap-0 h-full w-[95%] text-center align-middle">
                                                            <img  src={`${HeaderLogValChats.ProfilePic}`}  className={` flex cursor-pointer rounded-md mx-auto my-auto w-16 h-16 sm:min-h-fit sm:max-h-24 sm:max-w-24 sm:min-w-fit  `}  alt="" />
                                                            <span className=" mb-1 w-full justify-around gap-3 pl-2  flex flex-row align-middle">
                                                                <p  className=" min-w-[50px] sm:min-w-[100px] w-fit text-slate-200 my-auto text-left mr-auto align-middle">Name: </p>
                                                                <input   className="h-fit my-auto text-slate-200 text-sm outline-none bg-transparent text-left w-fit  border-none text-ellipsis" disabled type="text" value={HeaderLogValChats.Name} />
                                                            </span>
                                                            <span className=" mb-1 w-full justify-around gap-3 pl-2  flex flex-row align-middle">
                                                                <p  className=" min-w-[50px] sm:min-w-[100px] w-fit  text-slate-200 my-auto text-left mr-auto align-middle">Created On: </p>
                                                                <input   className="h-fit my-auto text-slate-200 text-sm outline-none bg-transparent text-left w-fit  border-none text-ellipsis" disabled type="text" value={HeaderLogValChats.createdOn} />
                                                            </span>
                                                            <span className=" mb-1 w-full justify-around gap-0 pl-2  flex flex-row align-middle">
                                                                <p  className=" w-fit min-w-fit max-w-[110px] text-slate-200 my-auto mr-auto text-left align-middle">{'About:' } </p>
                                                                <textarea disabled   className="h-fit  text-slate-200 text-sm outline-none bg-transparent min-w-[50%] text-left w-fit my-auto border-none min-h-[50px] max-h-[100px] max-w-[300px]: text-ellipsis" value={HeaderLogValChats.about} />
                                                            </span>
                                                            <span className=" mb-1 w-full  justify-around gap-3 pl-2  flex flex-row align-middle">
                                                                <p  className=" w-fit min-w-fit sm:min-w-fit  text-slate-200 my-auto text-left mr-auto align-middle">Mimimize: </p>
                                                                <button id="PoppinN"  className="h-fit bg-red-500 my-auto hover:text-slate-900 hover:bg-purple-800 font-semibold text-sm ml-auto md:text-sm   px-1 py-1 rounded-sm text-slate-800 outline-none text-left w-fit  border-none text-ellipsis" onClick={() => SetShowChatComponent('Mimimize')} >Mimimize</button>
                                                            </span>
                                                            <span className=" mb-1 w-full justify-around gap-3 pl-2  flex flex-row align-middle">
                                                                <p  className=" w-fit min-w-[50px] sm:min-w-[100px]  text-slate-200 my-auto text-left mr-auto align-middle">Close Chat: </p>
                                                                <button id="PoppinN"  className="h-fit bg-red-500 my-auto hover:text-slate-900 hover:bg-purple-800 font-semibold text-sm ml-auto  px-1 py-1 rounded-sm text-slate-800 outline-none text-left w-fit  border-none text-ellipsis" onClick={CloseChat} >Close</button>
                                                            </span>
                                                        </div>
                                                        <span onClick={() => SetDisplayMore(false)} className={` cursor-pointer w-fit h-fit text-lg hover:text-amber-500 text-slate-100 px-2  ml-auto sm:-translate-x-4 mx-auto text-center`}>&times;</span>
                                                </div>
                                        </div>                                        

                                    </div>
                                    {/* chat log */}
                                    <div  id="ChatlogContainer" ref={ChatlogContainer} className={` z-20 bg-transparent flex flex-col overflow-auto w-full gap-1 border-y-[1px] dark:border-slate-300 min-h-[250px] max-h-[250px] border-slate-800 h-[85%]  `}>
                                        {ChatLogMapper}
                                    </div>
                                    {/* text bar */}
                                    
                                    <div className={` z-[110%]  bg-transparent w-full flex relative flex-col ${IsStartChat ? ' py-0' : 'py-2'} justify-between h-fit min-h-fit `} >
                                            <div className={` ${IsStartChat ? 'hidden' : 'flex flex-col md:flex-row'} w-full   justify-start`} >
                                            ...
                                            <div  className={` ${IsStartChat ? 'hidden' : 'flex flex-row'}  bg-transparent w-full  py-2 justify-between h-fit min-h-fit z-30 `}>                                             
                                               <textarea 
                                                ref={ChatMessageRef}
                                                onChange={WriteGeneraldata}
                                                ...
                                                <div className=" text-right  grid sm:flex sm:flex-row sm:mr-3 sm:gap-4 grid-cols-2 gap-0 justify-end mr-2 ml-auto  w-[40%] max-w-[300px]">
                                                    ...
                                                    < FaLink onClick={PastDataText} title="Paste" className=" hover:text-xl ml-2 transition-all duration-300 hover:text-blue-600 cursor-pointer text-lg  my-auto"/>
                                                    
                                                    < IoSendSharp disabled={Generaldata.chatMessage == ''} onClick={() => SendChat('null','null','null')} title="send" className= {` ${Generaldata.chatMessage != '' ? " translate-x-0 z-30" : ' translate-x-[1000%] z-0 '} hover:text-xl hover:text-red-600 duration-300 transition-all cursor-pointer text-lg  my-auto `}/>
                                                </div>
                                            </div>
                                            <div className={` ${!IsStartChat ? 'hidden' : 'flex flex-row'} `}>
                                                <button onClick={StartChatFunc} className=" bg-sky-600 hover:text-slate-100 text-slate-800 w-full min-h-full dark:text-slate-100 dark:bg-blue-800 hover:bg-purple-700 dark:hover:bg-purple-700 transition-all duration-300 font-semibold  " >Start Chat</button>
                                            </div>
               ...
               <div onClick={() => ExpandChatComponent(true)} title="Expand" className= {` cursor-pointer ${ShowChatComponent == 'Mimimize' ? ' flex' : 'hidden'}  dropdown dropdown-top lg:dropdown-right  dropdown-end ml-0 left-[80%] sm:left-[90%]  top-auto md:left-[1%] fixed bottom-20 z-50 `}>
                    <img className=" rounded-full max-w-12 " src={HeaderLogValChats.ProfilePic} alt="" />
                </div> 
                <div className= {` dropdown dropdown-top lg:dropdown-right   dropdown-end ml-0 left-[80%] sm:left-[90%]  top-auto md:left-[1%] fixed bottom-2 z-50 `}>
                    ...
                    <ul tabIndex={0} className=" z-50 dropdown-content dark:ring-0 ring-2 ring-sky-700  menu py-2 shadow bg-base-100 rounded-md gap-2 font-semibold w-52">
                        <li onClick={ToogleChat} className=" w-full hover:text-amber-600 p-2 cursor-pointer hover:bg-slate-800 transition-all duration-300" >Messager</li>
                    </ul>
                </div>
