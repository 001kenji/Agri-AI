...
const Navbar = ({logout, RefreshRequest, isAuthenticated, GetCSRFToken}) => {
    ...
    function CloseWebsockets() {
        ...
        if (window.WebSocket) {
            // Iterate over each WebSocket instance created in the browser
    ...
    const Authlink = () => (
        ...
        <Fragment>
            <Link onClick={LogoutFunc} to="/login"><li 
             className=' my-auto lg:my-auto md:bg-blue-600 md:py-1 md:px-2 hover:bg-red-500 transition-all duration-300 '>Log-Out</li>
            </Link>
            
            <Link to="/home" className=' lg:my-auto '><li className="hover:bg-blue-600 transition-all duration-300 hover:text-slate-50 px-2 py-1 group-hover:text-slate-50" >Home</li></Link>
            <Link to="/chat" className=' lg:my-auto'><li className="hover:bg-blue-600 transition-all duration-300 hover:text-slate-50 px-2 py-1 group-hover:text-slate-50"  >Chat</li></Link>
            <Link to="/jobs" className=' lg:my-auto'><li className="hover:bg-blue-600 transition-all duration-300 hover:text-slate-50 px-2 py-1 group-hover:text-slate-50"  >Jobs</li></Link>
        </Fragment>
                ...
                <div className="transition-all duration-300 dark:border-b-sky-600 border-b-[1px]" id='navbar-contaner-div'>
                    <div className=" flex flex-row-reverse justify-around" id='top-nav-div'>
                        <div id='top-nav-icon-div'>
                            <CiMenuFries className=' dark:text-orange-500 dark:hover:text-lime-400 transition-all duration-300 z-[100%] cursor-pointer hover:text-sky-700 text-slate-900 text-3xl' onClick={ShowNav} />
                        </div>
